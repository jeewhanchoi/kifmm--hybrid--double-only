...
{
  int i, j;
	real_t* mat = reals_alloc__aligned (trg.num_pts*src.num_pts);
  SIMD_REG OOFP_R__ = SIMD_SET1 (OOFP_R);  
  for (i = 0; i < src.num_pts; i++) {
    SIMD_REG SX__ = SIMD_LOAD1 (src.x + i);
    SIMD_REG SY__ = SIMD_LOAD1 (src.y + i);
    SIMD_REG SZ__ = SIMD_LOAD1 (src.z + i);
    for (j = 0; (j+SIMD_LEN) <= trg.num_pts; j+=SIMD_LEN) {
      SIMD_REG DX1__ = SIMD_SUB (SX__, SIMD_LOAD_U (trg.x + j));
      SIMD_REG DY1__ = SIMD_SUB (SY__, SIMD_LOAD_U (trg.y + j));
      SIMD_REG DZ1__ = SIMD_SUB (SZ__, SIMD_LOAD_U (trg.z + j));
...
//      R__ = SIMD_INV_SQRT (R__);
      ...
      SIMD_STORE_U (&mat[i*trg.num_pts+j], R__);
    }
  for (; j < trg.num_pts; j++) {
      real_t x = src.x[i] - trg.x[j];
      real_t y = src.y[i] - trg.y[j];
      real_t z = src.z[i] - trg.z[j];
      ...
      real_t r = sqrtf(r2);
#endif
      mat[i*trg.num_pts+j] = OOFP_R / r;
  }
  }
  dgemv (trg.num_pts, src.num_pts, 1.0, mat, src.den_pot, 1.0, trg.den_pot);
  reals_free (mat);
