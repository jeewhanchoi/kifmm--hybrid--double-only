...
#pragma omp parallel private(eden, eval, UpwEqu2DwnChkii) 
    {
      int i, j, b;
      int st_blk, end_blk;
      int bs = getenv__block_size ();
      int trans_vec_count = 7*7*7;
      int nb = trans_vec_count / bs;
      int tid = omp_get_thread_num();
      for (b = 0; b < nb; b++) {
        st_blk = b * bs;
        end_blk = (b == nb - 1) ? trans_vec_count : (b + 1) * bs;
        for (i = Ti[tid]; i < Ti[tid+1]; i++) {
          if( nodeVec[i].tag & LET_TRGNODE && nodeVec[i].Vnodes.size()>0) { 
...
wlist_calc__cpu (FMMWrapper_t *F)
{
  ...
  for (int i = 0; i < nthreads; i++) {
    ...
    sl_pos[i]->x = (real_t *) reals_alloc__aligned (SP[UE].n);
    sl_pos[i]->y = (real_t *) reals_alloc__aligned (SP[UE].n);
    sl_pos[i]->z = (real_t *) reals_alloc__aligned (SP[UE].n);
    sl_pos[i]->num_pts = SP[UE].n;
  }
      ...
      if( nodeVec[i].child == -1) { 
        for (int j = 0; j < nodeVec[i].Wnodes.size(); j++) {
          ...
          if(nodeVec[src].child == -1 && nodeVec[src].srcNum*srcDOF<pln_size(UE, SP)) { 
            ...
            int tid = omp_get_thread_num();
            compute_localpos (c, r, sl_pos[tid], &SP[UE]);
            sl_pos[tid]->den_pot = &All_N->src_upw_equ_den[src*pln_size(UE, SP)];
...
xlist_calc__cpu (FMMWrapper_t* F)
{
  ...
  for (int i = 0; i < nthreads; i++) {
    ...
    tl_pos[i]->x = (real_t *) reals_alloc__aligned (SP[DC].n);
    tl_pos[i]->y = (real_t *) reals_alloc__aligned (SP[DC].n);
    tl_pos[i]->z = (real_t *) reals_alloc__aligned (SP[DC].n);
    tl_pos[i]->num_pts = SP[DC].n;
  }
    ...
    if( nodeVec[i].tag & LET_TRGNODE) { 
      for (int j = 0; j < nodeVec[i].Xnodes.size(); j++) {
        ...
        if(nodeVec[i].child == -1 && nodeVec[i].trgNum*trgDOF<pln_size(DC, SP)) { 
          ...
          r = radius (i, nodeVec);
          int tid = omp_get_thread_num();
          compute_localpos (c, r, tl_pos[tid], &SP[DC]);
          tl_pos[tid]->den_pot = &All_N->trg_dwn_chk_val[i*pln_size(DC, SP)];

          ulist__direct_evaluation (*tl_pos[tid], All_N->Ns[src]);
        }
      }
    }
  }
